...
def main():
    ...
    lgbm_group.to_csv("outputs/metrics/lightgbm_group_metrics.csv", index=False)
    cat_group.to_csv("outputs/metrics/catboost_group_metrics.csv", index=False)

    plot_actual_vs_pred(
        test_predictions.head(400),
        "pred_lightgbm",
        "LightGBM Actual vs Predicted",
        "outputs/figures/lightgbm_actual_vs_pred.png",
    )
    plot_error_distribution(
        test_predictions,
        "pred_lightgbm",
        "LightGBM Error Distribution",
        "outputs/figures/lightgbm_error_distribution.png",
    )
    plot_group_metric_bar(
        lgbm_group,
        "MAE",
        "Top Group MAE for LightGBM",
        "outputs/figures/lightgbm_group_mae.png",
    )

    logger.info("Saved baseline metrics, group metrics, and plots.")
